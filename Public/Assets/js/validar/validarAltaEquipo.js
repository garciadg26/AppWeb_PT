let formulario = document.getElementById('form_alta_equipo');
const inputs = document.querySelectorAll('#form_alta_equipo input');

const expresiones = {
    nomEquipoINP: /^[\w\s\u00C0-\u024F]{2,60}$/, // Letras y espacios, pueden llevar acentos.
    numSerieINP: /^[1-9]\d{5,8}$/, // 10 a 14 numeros.
...
const validarFormulario = (e) => {
    switch (e.target.name){
        case "nomEquipoINP":
            validarCampo(expresiones.nomEquipoINP, e.target, "nomEquipoINP");
        break;
        case "numSerieINP":
            validarCampo(expresiones.numSerieINP, e.target, "numSerieINP");
        break;
    }
}
...
formulario.addEventListener('submit', e=>{
    ...
    let estatusEquipo = document.getElementById('statusEquipoINP');
    ...
    if(campos.nomEquipoINP && campos.numSerieINP && estatusEquipo.value != ""){
        ...
        // fetch('https://ritchman.com/altaComputo/crearEquipo', peticion)
        fetch('http://localhost/iam/altaComputo/crearEquipo', peticion)
